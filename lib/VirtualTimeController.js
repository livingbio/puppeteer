...
class VirtualTimeController {
   ...
   *
   * @param {!function(*)} startDeferrer
   */
  setStartDeferrer(startDeferrer) {
    this._startDeferrer = startDeferrer;
  }
   ...
   *
   * If registered, the StartDeferrer can delay the start until some action has completed.
   */
  startVirtualTime() {
    ...
    if (this._startDeferrer)
      this._startDeferrer(this._setVirtualTimePolicy.bind(this, nextBudget, waitForNavigation));
    else
   ...
   */
  grantVirtualTimeBudget(budget, startPolicy) {
    const controller = this;
